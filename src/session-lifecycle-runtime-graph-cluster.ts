...
interface SessionLifecycleRuntimeGraphClusterDeps {
  ...
  resolveSessionMicHoldCalibrationLevel: (input: { trainingMode: string; inputSource: 'microphone' | 'midi' }) => typeof state.performanceMicHoldCalibrationLevel;
  ...
  ensureAudioRuntime: (runtimeState: typeof state, options: { audioInputDeviceId: string | null; analyserProfile: 'default' | 'low-latency-performance' }) => Promise<void>;
