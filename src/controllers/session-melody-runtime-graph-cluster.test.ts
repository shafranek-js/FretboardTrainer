...
vi.mock('./session-melody-settings-cluster', () => ({
  createSessionMelodySettingsCluster,
}));
vi.mock('./session-melody-timeline-editing-cluster', () => ({
  createSessionMelodyTimelineEditingCluster,
}));
vi.mock('./session-runtime-ui-cluster', () => ({
  createSessionRuntimeUiCluster,
}));
vi.mock('./session-melody-demo-cluster', () => ({
  createSessionMelodyDemoCluster,
}));
...
describe('session-melody-runtime-graph-cluster', () => {
  it('wires settings, runtime-ui, and melody-demo callbacks through the graph layer', async () => {
    ...
    createSessionMelodySettingsCluster.mockReturnValue({
      selectedMelodyContextController: { id: 'selected' },
      melodyPracticeSettingsController: { id: 'practice' },
      melodyPracticeSettingsBridgeController: { id: 'bridge' },
    });
    ...
    createSessionMelodyDemoCluster.mockReturnValue({
      melodyDemoRuntimeController: { clearPreviewState, stopPlayback },
      sessionTransportControlsController: { id: 'transport' },
    });

    const { createSessionMelodyRuntimeGraphCluster } = await import('./session-melody-runtime-graph-cluster');
    ...
    demoArgs.sessionTransportControls.applyUiWorkflow('perform');
