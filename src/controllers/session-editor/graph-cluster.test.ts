import type { createSessionEditorGraphCluster as CreateSessionEditorGraphCluster } from './graph-cluster';
...
describe('session-editor-graph-cluster', () => {
  it('wires editor and library graph callbacks through the session editor graph', async () => {
    ...
    const { createSessionEditorGraphCluster } = await import('./graph-cluster');
