...
type MelodyImportModalState = Parameters<typeof createMelodyImportModalController>[0]['state'];
type MelodyImportEditorClusterState = Pick<AppState, 'melodyEditorMode' | 'editingMelodyId'>;
...
interface MelodyImportEditorClusterDeps {
  dom: AppDom;
  state: MelodyImportEditorClusterState;
...
export function createMelodyImportEditorCluster(deps: MelodyImportEditorClusterDeps) {
  const melodyImportModalState = createMelodyImportModalState(deps.state);
  ...
  const melodyLibraryActionsController = createMelodyLibraryActionsController({
    getSelectedMelody: deps.getSelectedMelody,
    getCurrentInstrument: deps.getCurrentInstrument,
    getMelodyEditorMode: () => melodyImportModalState.melodyEditorMode,
    getEditingMelodyId: () => melodyImportModalState.editingMelodyId,
...
function createMelodyImportModalState(
  state: MelodyImportEditorClusterState
): MelodyImportModalState {
  const modalState = {} as MelodyImportModalState;

  Object.defineProperties(modalState, {
    melodyEditorMode: {
      enumerable: true,
      get: () => state.melodyEditorMode,
      set: (value: MelodyImportModalState['melodyEditorMode']) => {
    ...
    editingMelodyId: {
      ...
      set: (value: MelodyImportModalState['editingMelodyId']) => {
