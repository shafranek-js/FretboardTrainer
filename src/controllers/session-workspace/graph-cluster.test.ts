import type { createSessionWorkspaceGraphCluster as CreateSessionWorkspaceGraphCluster } from './graph-cluster';
...
describe('session-workspace-graph-cluster', () => {
  it('wires cyclic preset-ui and workspace control callbacks through the workspace graph', async () => {
    ...
    const { createSessionWorkspaceGraphCluster } = await import('./graph-cluster');
